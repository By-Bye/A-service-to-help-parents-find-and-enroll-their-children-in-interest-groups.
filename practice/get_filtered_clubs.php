...
if (isset($_POST['categories']) && isset($_POST['price_filter']) && isset($_POST['search_query'])) {
    $categories_json = $_POST['categories'];
    $selected_categories = json_decode($categories_json, true);
    $price_filter = $_POST['price_filter'];
    $search_query = trim($_POST['search_query']); // Получаем и очищаем поисковый запрос
    ...
    if (!empty($search_query)) {
        ...
        $where_clause .= "(c.title LIKE ? OR c.hashtag LIKE ? OR c.description LIKE ?)";
